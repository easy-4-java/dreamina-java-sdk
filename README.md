...
`dreamina-java-sdk` on the `feature/1.0.x` line is a JDK 8, Spring-free SDK that drives the official Dreamina command line interface (`dreamina` CLI). It wraps subprocess execution, parses structured output (JSON / text / tables), and exposes strongly typed request and result objects for every built-in and generator command of the CLI.
...
| jackson-databind | 2.18.9 (declared via Jackson BOM) |

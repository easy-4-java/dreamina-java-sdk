...
`dreamina-java-sdk` on the `feature/2.0.x` line is a JDK 17, Spring-free SDK that drives the official Dreamina command line interface (`dreamina` CLI). It wraps subprocess execution, parses structured output (JSON / text / tables), and exposes strongly typed request and result objects for every built-in and generator command of the CLI.
...
|:---|:---|
| JDK | 17 |
| Maven | Use the included wrapper (`./mvnw`, Maven 3.9.16) |
| Local CLI | Official `dreamina` CLI (see Installation) |
| jackson-databind | 2.22.1 (declared via Jackson BOM) |
...
```groovy
implementation 'io.github.easy4j:dreamina-java-sdk:2.0.x.x.20260630-SNAPSHOT'
```
